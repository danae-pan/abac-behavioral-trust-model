...
def merge_school_and_state_data(school_data_path: str, state_summary_path: str, output_path: str):
    ...
    """
    if not os.path.exists(school_data_path) or not os.path.exists(state_summary_path):
    ...
    for idx, school in enumerate(flat_schools, 1):
        if not school or "state_name" not in school or not school["state_name"]:
            print(f"[Row {idx}] Skipping: missing state name.")
            continue
